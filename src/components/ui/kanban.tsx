...
function KanbanColumn({
  ...
  const { activeId, isColumn, findContainer } = React.useContext(KanbanContext);
  ...
  return (
    ...
    >
      <div
        ...
        data-over={isItemOver || undefined}
        ref={setNodeRef}
        style={style}
        className={cn(
...
function KanbanItem({
  ...
  const { activeId, isColumn, sortable } = React.useContext(KanbanContext);
  const isItemDragging = activeId ? !isColumn(activeId) : false;

  const style = (
    sortable ? { transition, transform: CSS.Translate.toString(transform) } : {}
  ) as React.CSSProperties;
